...
CREATE TABLE users(userid int NOT NULL AUTO_INCREMENT, email text, name text, password text, verified tinyint(4) DEFAULT 0, admin tinyint(4) NOT NULL DEFAULT 0, primary key(userid));
...
CREATE TABLE events(name text, method text, day text, time_of_execution text, event_enabled boolean DEFAULT true, event_data text);
...
GRANT INSERT, SELECT, UPDATE ON event_data TO pythonhttp@localhost;